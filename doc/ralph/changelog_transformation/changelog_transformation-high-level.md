...
- **Behavior: Create Auto-Creates Directory**
  - GIVEN no `./_change_log/` directory exists in any parent
  - AND the current directory is inside a git repository
  - WHEN `change_log create "First entry" --impact 1`
  - THEN `./_change_log/` is created at the git repo root
...
| `.tickets/` directory | Replaced by `./_change_log/` | Confirmed |
